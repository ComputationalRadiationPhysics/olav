...
void DotSceneLoader::parseDotScene(const String &SceneName, const String &groupName, SceneManager *yourSceneMgr, SceneNode *pAttachNode, const String &sPrependNode)
{
    ...
    // Close the XML File
    delete XMLDoc;
    OgreFramework::getSingletonPtr()->m_pLog->logMessage("[DotSceneLoader] Parsed " + Ogre::StringConverter::toString(entity_count) + " entities");
    OgreFramework::getSingletonPtr()->m_pLog->logMessage("[DotSceneLoader] Parsed " + Ogre::StringConverter::toString(node_count) + " nodes");
}
